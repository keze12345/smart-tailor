...
class _TailorDashboardScreenState extends State<TailorDashboardScreen>
          ...
          children: [
            ...
            TextField(
                ...
                decoration: const InputDecoration(
                    labelText: 'Phone',
                    prefixIcon:
                        Icon(Icons.phone_outlined, color: Color(0xFF1B5E20)))),
            const SizedBox(height: 12),
            TextField(
                ...
                decoration: const InputDecoration(
                    ...
                    prefixIcon:
                        Icon(Icons.email_outlined, color: Color(0xFF1B5E20)))),
            const SizedBox(height: 12),
            TextField(
                ...
                decoration: const InputDecoration(
                    ...
                    prefixIcon:
                        Icon(Icons.notes_outlined, color: Color(0xFF1B5E20)))),
  ...
  void _showMeasurementsSheet(Map customer) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      shape: const RoundedRectangleBorder(
                ...
                children: [
                  ...
                  Expanded(
                      child: Text(order['note'],
                          style: const TextStyle(
                              fontSize: 13, color: Color(0xFF3A3A3C)))),
                ],
              ),
            ),
          ],
              ...
              child: TabBarView(
                ...
                children: [
                  _loadingOrders
                      ...
                      : _pending.isEmpty
                          ? _buildEmptyState(
                              'No pending orders', Icons.inbox_outlined)
                          : RefreshIndicator(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (ctx, i) =>
                                    _buildOrderCard(_pending[i], true),
                              ),
                            ),
                  _loadingOrders
                      ? const Center(
                          child: CircularProgressIndicator(
                              color: Color(0xFF1B5E20)))
                      : _history.isEmpty
                          ? _buildEmptyState('No order history yet',
                          ...
                          : RefreshIndicator(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (ctx, i) =>
                                    _buildOrderCard(_history[i], false),
                              ),
                            ),
                  _loadingCustomers
                      ? const Center(
...
class _MeasurementsSheetState extends State<_MeasurementsSheet> {
  ...
  void _showAddMeasurement({Map? existing}) {
    final chestCtrl =
        TextEditingController(text: existing?['chest']?.toString() ?? '');
    final waistCtrl =
        TextEditingController(text: existing?['waist']?.toString() ?? '');
    final hipsCtrl =
        TextEditingController(text: existing?['hips']?.toString() ?? '');
    final shoulderCtrl =
        TextEditingController(text: existing?['shoulder']?.toString() ?? '');
    final sleeveCtrl =
        TextEditingController(text: existing?['sleeve']?.toString() ?? '');
    final inseamCtrl =
        TextEditingController(text: existing?['inseam']?.toString() ?? '');
    final labelCtrl = TextEditingController(
        text: existing?['label'] ?? 'Manual Entry');
    final isEdit = existing != null;
      ...
      builder: (ctx) => Padding(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(isEdit ? 'Edit Measurements' : 'Add Measurements',
                  style: const TextStyle(
                      fontSize: 20,
                      fontWeight: FontWeight.bold,
                      color: Color(0xFF1C1C1E))),
              ...
              Row(children: [
                ...
                Expanded(
                    child: TextField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'Waist'))),
              ]),
              const SizedBox(height: 12),
              Row(children: [
                Expanded(
                    child: TextField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'Hips'))),
                const SizedBox(width: 12),
                Expanded(
                    child: TextField(
              ...
              Row(children: [
                ...
                Expanded(
                    child: TextField(
                        controller: inseamCtrl,
                        keyboardType: TextInputType.number,
                        decoration:
                            const InputDecoration(labelText: 'Inseam'))),
              ]),
              const SizedBox(height: 24),
              SizedBox(
                width: double.infinity,
                child: ElevatedButton(
                  onPressed: () async {
                    final body = jsonEncode({
                      ...
                      'shoulder':
                          double.tryParse(shoulderCtrl.text) ?? 0,
                    ...
                    if (isEdit) {
                      await http.put(
                        Uri.parse(
                            '$_base/api/tailor-customers/measurements/${existing['id']}'),
                        headers: {'Content-Type': 'application/json'},
                        body: body,
                      );
                    } else {
          ...
          _loading
              ...
              : _measurements.isEmpty
                  ? const Center(
                      ...
                      child: Text('No measurements yet. Tap Add to record.',
                          textAlign: TextAlign.center,
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Wrap(
                                  ...
                                  runSpacing: 4,
                                  children: [
                                    _measureChip('Chest', m['chest']),
                                    _measureChip('Waist', m['waist']),
                                    _measureChip('Hips', m['hips']),
                                    _measureChip(
                                        'Shoulder', m['shoulder']),
                                    _measureChip('Sleeve', m['sleeve']),
                                    _measureChip('Inseam', m['inseam']),
                                  ],
                                ),
                              ],
                            ),
  ...
  Widget _measureChip(String label, dynamic value) {
    return Container(
      padding:
          const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
      decoration: BoxDecoration(
      ...
      child: Text('$label: ${value ?? 0}cm',
          style: const TextStyle(
              fontSize: 12, color: Color(0xFF3A3A3C))),
    );
  }
}
